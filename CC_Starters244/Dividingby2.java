...
public class Dividingby2{
    ...
    static class Pair{
        long val;
        long step;
        public Pair(long val,long step){
            this.step=step;
            this.val=val;
        }
    }
    ...
    static void solve() {
       ...
       for (int i=0;i<n;i++){
        ...
        long common=-1;
        ArrayList<Pair>first=hashMap.get(arr[0]);
        for (Pair p:first){
            long candidate=p.val;
            boolean ok=true;
            for (int i=1;i<n&&ok;i++) {
                boolean found=false;
                for (Pair q:hashMap.get(arr[i])){
                    if (q.val==candidate){
                        found=true;
                        break;
                    }
        ...
        for (long k:hashMap.keySet()) {
            long value=k;
            ArrayList<Pair> list=hashMap.get(k);
            for (Pair p : list) {
                if (p.val==common){
                    ans += p.step * cnt.get(value);
                    break;
                }
            }
        }
